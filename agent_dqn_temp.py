...
class DQNAgent:
    ...
    def replay(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            ...
            state_array = np.array(state[0:3], dtype=np.float64)
            next_state_array = np.array(next_state[0:3], dtype=np.float64)

            state_tensor = tf.constant(state_array)
            next_state_tensor = tf.constant(next_state_array)

            state_tensor = tf.reshape(state_tensor, (1,3))
            next_state_tensor = tf.reshape(next_state_tensor, (1,3))
            ...
            target_f[0][action] = target
            self.model.fit(state[0:3], target_f, epochs=1, verbose=0)
        if self.epsilon > self.epsilon_min:
            #print('hit')
            print(self.epsilon)
            self.epsilon *= self.epsilon_decay
