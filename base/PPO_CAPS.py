...
class PPO_discrete_CAPS(object):
    def __init__(
            self,
            env_with_Dead,
            state_dim,
    ...
    def train(self):
        ...
        for _ in range(self.K_epochs):
            ...
            for i in range(optim_iter_num):
                ...
                '''actor update'''
                #print('prob',s[index], 'len', len(s[index]), 'shape', s[index].shape)
                ...
                #print('mu', mu)
                x_nxt = s_prime[index]
                x_bar = psi(s[index], s[index].size(1))
                prob_nxt = self.actor.pi(x_nxt, softmax_dim=1)
                prob_bar = self.actor.pi(x_bar, softmax_dim=1)
                #print('x_nxt', x_nxt, 'x_bar', x_bar)
                #print('s len', len(s[index]), 'x nxt', len(x_nxt), 'x_bar', len(x_bar))
                mu_nxt = torch.log(prob_nxt)
                mu_bar = torch.log(prob_bar)

                #print('mu_nxt', mu_nxt.shape, 'mu_bar', mu_bar.shape, 'a_loss', a_loss.shape, 'mu', mu.shape)
                #sys.exit(1)
                # reg_a
                a_loss += self.lambda_a * torch.mean(torch.square(mu_nxt - mu)) / float(mu.size(0))
                # reg_c
                a_loss += self.lambda_s * torch.mean(torch.square(mu_bar - mu)) / float(mu.size(0))
