...
if __name__ == "__main__":
    ...
    with open('./temp.txt', 'r') as handle:
        no_demand_response = eval(handle.read())

    demand_response = no_demand_response[:]
    print(len(demand_response))
    for i in range(len(demand_response)):
        demand_response[i] = demand_response[i] + random.uniform(-0.1, 0.0)
