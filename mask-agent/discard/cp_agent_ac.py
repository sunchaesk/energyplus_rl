...
def sample(mean, variance, mask_range):
    ...
    actions = torch.clamp(actions, mask_range[0], mask_range[1])
...
for ep in range(start_episode, max_episodes + 1):
    ...
    while not done:
        ...
        if env.b_during_sim():
            ...
            action, logprob, entropy = sample(mean.cpu(), variance.cpu(), env.calc_mask_range(state))
