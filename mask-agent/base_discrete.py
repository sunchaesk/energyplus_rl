...
[X] TODO: upload energyplus and start porting to google colab Yay
  -  [X] setup gym, and E+ env on google colab
  - [ ] Finish porting and test AC on colab
[ ] extend base.py to base_base.py, base_continuous.py, base_discrete.py -> change action spaces
[X] github issue about premature ending if simulation not annual
[X]  change base.py so the simulation is only for 6/21->8/21
[  ] make it so that if reward==0, then skip the NN computation
...
if __name__ == "__main__":
    ...
    for episode in range(1):
        ...
        while not done:
            ...
            print('DATE', info['date'][0], info['date'][1], 'REWARD:', reward, 'ACTUATOR VAL', action)
            #print('OBS', n_state)
            #print('RET STUFF:', ret)
            score+=reward
